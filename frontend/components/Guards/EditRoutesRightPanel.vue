...
<script>
import TimePickerCommon from "../../components/Common/TimePickerCommon.vue";
export default {
  components: {
    TimePickerCommon,
  },
  ...
  created() {
    ...
    const hours = Array.from({ length: 11 }, (_, i) => {
      const value = (i + 1) * 60; // Convert hours to minutes
      return { text: `${i + 1} hour${i + 1 > 1 ? "s" : ""}`, value };
    });
  ...
  methods: {
    ...
    save_device_info() {
      ...
      let payload = {
        ...
        route_start_hour: this.routeStartTime.split(":")[0],
        route_start_minute: this.routeStartTime.split(":")[1],
        route_end_hour: this.routeEndTime.split(":")[0],
        route_end_minute: this.routeEndTime.split(":")[1],
